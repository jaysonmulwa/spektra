 ...
 */
function degreeMatrix(edges: [number, number][], numVertices: number): twoDmatrix {
 ...
 */
export function laplacian(edges: [number, number][], numVertices: number): twoDmatrix {
  const D = degreeMatrix(edges, numVertices);
  const A = adjacency(edges, numVertices);
  const L: twoDmatrix = Array(numVertices).fill(0).map(() => Array(numVertices).fill(0));
